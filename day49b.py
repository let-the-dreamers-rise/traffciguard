"""Stronger day-49 model (the winning lever: trained on day-49 labels).
Richer day-48 reference features (exact+coarse lookups, lags, SVD, spatial KNN),
day-49's own weather/time/structure, 5-seed ensemble. Then blend with submission_best.
Writes submission_day49b.csv + several weighted blends. No test labels used."""
...
# day-48 reference features
R_ghts=d48.groupby(['geohash','timestamp'])['demand'].mean()
R_gh5ts=d48.groupby(['gh5','timestamp'])['demand'].mean()
R_gh4ts=d48.groupby(['gh4','timestamp'])['demand'].mean()
R_gh=d48.groupby('geohash')['demand'].mean(); R_ts=d48.groupby('timestamp')['demand'].mean()
R_ghslot=d48.groupby(['geohash','slot'])['demand'].mean()
...
def add_feats(df):
    ...
    df['sp_knn']=np.where(np.isnan(SP[df.geohash.map(gi).values,df.timestamp.map(ti).values]),GM,
                          SP[df.geohash.map(gi).values,df.timestamp.map(ti).values])
...
FEAT=['lat','lon','mins','slot','hour','t_sin','t_cos','RoadType','NumberofLanes','LV','LM','Temperature','Weather',
      'r_ghts','r_gh5ts','r_gh4ts','r_gh','r_ts','r_ghhour','rp1','rn1','rp2','rn2','rp4','rn4','sp_knn','svd_recon']+[f'ghfac{k}' for k in range(6)]
...
d49pred=np.clip(tp,0,1)
pd.DataFrame({'Index':test.Index.values,'demand':d49pred}).to_csv("submission_day49b.csv",index=False)
print("wrote submission_day49b.csv mean=",round(d49pred.mean(),4))

best=pd.read_csv("submission_best.csv").set_index('Index')['demand']; bb=test.Index.map(best).values
for w in [60,70,80]:
    bl=np.clip((w/100)*d49pred+(1-w/100)*bb,0,1)
    pd.DataFrame({'Index':test.Index.values,'demand':bl}).to_csv(f"submission_blend_{w}.csv",index=False)
    print(f"wrote submission_blend_{w}.csv mean=",round(bl.mean(),4))
