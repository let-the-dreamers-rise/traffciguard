...
for w in [66,70,74]:
    bl=np.clip((w/100)*d49e+(1-w/100)*bb,0,1)
    pd.DataFrame({'Index':test.Index.values,'demand':bl}).to_csv(f"submission_e_blend_{w}.csv",index=False)
    print(f"wrote submission_e_blend_{w}.csv mean=",round(bl.mean(),4))
